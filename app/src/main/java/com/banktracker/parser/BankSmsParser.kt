...
object BankSmsParser {
    private val BANK_SENDERS = setOf(
        ...
        "HDBank","Eximbank","8149","8150","8900","6088"
    )
    private val AMOUNT_PATTERNS = listOf(
        Regex("""([\d,\.]+)\s*(?:VND|vnd|đ|VNĐ)"""),
        Regex("""(?:so tien|tien giao dich|amount)\s*:?\s*([\d,\.]+)""", RegexOption.IGNORE_CASE),
        Regex("""(?:PS No|PS Co|Tien vao|Tien ra|GD)\s*:?\s*([\d,\.]+)""", RegexOption.IGNORE_CASE),
        Regex("""(?:thanh toan|chi|nap)\s+([\d,\.]+)""", RegexOption.IGNORE_CASE)
    )
    private val DEBIT_WORDS = listOf("no","chi","thanh toan","rut tien","debit","giam","payment","ps no","tien ra")
    private val CREDIT_WORDS = listOf("co","nhan","chuyen den","credit","nap","tang","ps co","tien vao")
    ...
    private fun extractDescription(body: String): String {
        val p = Regex("""(?:ND|noi dung|GD|mo ta|desc|ref)\s*:?\s*(.{3,80})""", RegexOption.IGNORE_CASE)
    ...
    private fun detectBank(sender: String) = when {
        ...
        sender.contains("TPB",true)||sender.contains("TPBank",true) -> "TPBank"
        else -> sender
    }
}
